...
createBasketBtn = (count, container) => {
...
const createBasket = (container, arr) => {
  let sum = 0;

  arr.forEach((el) => {
    let basketItem = createElement({
      type: "div",
      attrs: { class: "basket__item-container", datacode: `${el.productCode}` },
      container,
    });
    ...
    createElement({
      type: "span",
      attrs: { class: "basket__item-count", innerText: `- ${el.count} шт` },
      container: basketItem,
    });
    createElement({
      type: "span",
